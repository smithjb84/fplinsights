...
function initialsetup() {
	$.get(chrome.extension.getURL('/charts.html'), function(data) {
		$( "#graph-area" ).remove();
		$($.parseHTML(data)).appendTo('.DeadlineBar__StyledDeadlineBar-sc-15w29wz-0');
		initiateSettings();
		var featureb = $( "#feature" ).val();
		var positionb = parseInt($( "#positionb" ).val());
...
function showPlayers() {
    var playerPics = setInterval(function () {
        if ($('.sc-bdnxRM').length) {
            ...
            $('.flTOZR').each(function (index) {
                var name = $(this).parent().parent().find('.PitchElementData__ElementName-sc-1u4y6pr-1').text();
				...
				console.log(code);
                var personpic = "//resources.premierleague.com/premierleague/photos/players/110x140/p" + code + ".png";
